...
export const useTagEvent = (
  ...
  const getCurrentRoute = (view?: RouteLocationNormalizedLoaded) => {
    const current =
      view || (isTab ? <RouteLocationNormalizedLoaded>tagItem : route)
    const index = unref(getTagList).findIndex(
      (item:any) => item.path === current.path,
    )
  ...
  const closeLeftDisabled = computed(() => {
    ...
    return (
      [0, -1].includes(index) || !!unref(getTagList)[index - 1]?.meta?.affix
    )
  })
  ...
  const closeRightDisabled = computed(() => {
    const { index } = getCurrentRoute()
    return (
      [unref(getTagList).length - 1, -1].includes(index) ||
      !!tagStore.tagList[index + 1]?.meta?.affix
    )
  })
  ...
  const closeOtherDisabled = computed(() => {
    ...
    return (
      ...
      unref(getTagList).filter((item, i) => !item?.meta?.affix && i !== index)
        .length === 0
    )
  })
  ...
  const closeAllDisabled = computed(() => {
    return (
      unref(getTagList).length === 1 ||
      unref(getTagList).filter((item) => !item?.meta?.affix).length === 0
    )
  })
