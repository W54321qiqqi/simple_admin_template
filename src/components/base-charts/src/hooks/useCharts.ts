...
export const useCharts = (elRef: Ref<HTMLDivElement>) => {
  ...
  const getOption = computed((): EChartsOption => {
    return unref(isDark)
      ? (Object.assign({}, unref(cacheOption), {
  ...
  const initCharts = () => {
    ...
    useResizeObserver(
      ...
      useDebounceFn((e) => {
        ...
        if (
          Math.round(contentRect.width) !== chartInstance?.getWidth() ||
          Math.round(contentRect.height) !== chartInstance?.getHeight()
        ) {
          resize()
        }
      }, 100),
